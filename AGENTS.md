...
- Core types in individual modules (`arch.rs`, `variant.rs`, `error.rs`)
...
Minimal: currently only `thiserror`. Any new dependency must be justified. Prefer standard library solutions where reasonable.
...
Minimum Supported Rust Version is **1.71**. CI tests against both stable and MSRV. Do not use features that require a newer version without updating `rust-version` in `Cargo.toml` and the CI matrix.
